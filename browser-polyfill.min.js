        ...
        f = (o, p) => {
          return (...q) => {
            chrome.runtime.lastError
              ? o.reject(chrome.runtime.lastError)
              : p.singleCallbackArg || 1 === q.length
              ? o.resolve(q[0])
              : o.resolve(q)
          }
        },
        g = (o, p) => {
          ...
          return function (s, ...t) {
            if (t.length < p.minArgs)
              throw new Error(
                `Expected at least ${p.minArgs} ${q(
                  p.minArgs
                )} for ${o}(), got ${t.length}`
              )
            if (t.length > p.maxArgs)
              throw new Error(
                `Expected at most ${p.maxArgs} ${q(
                  p.maxArgs
                )} for ${o}(), got ${t.length}`
              )
              ...
              get(t, u) {
                ...
                if (u in t) {
                  ...
                  if ('function' == typeof w) {
                    if ('function' == typeof p[u]) w = h(t, t[u], p[u])
                    else if (i(q, u)) {
                      let x = g(u, q[u])
                      w = h(t, t[u], x)
                    } else w = w.bind(t)
